...
class SimpleChatbot(Component):
    ...
    Also supports audio/video/image files, which are displayed in the SimpleChatbot, and other kinds of files which are displayed as links. This
    ...
    def __init__(
        ...
        *,
        ...
        Parameters:
            ...
            type: The format of the messages. If 'tuples', expects a `list[list[str | None | tuple]]`, i.e. a list of lists. The inner list should have 2 elements: the user message and the response message. The individual messages can be (1) strings in valid Markdown, (2) tuples if sending files: (a filepath or URL to a file, [optional string alt text]) -- if the file is image/video/audio, it is displayed in the SimpleChatbot, or (3) None, in which case the message is not displayed. If 'messages', passes the value as a list of dictionaries with 'role' and 'content' keys. The `content' key's value supports everything the 'tuples' format supports. The 'role' key should be one of 'user' or 'assistant'. Any other roles will not be displayed in the output.
    ...
    def _preprocess_content(
        ...
        if chat_message is None:
            ...
            if component is not None:
                ...
                raise ValueError(
                    f"Invalid component for SimpleChatbot component: {chat_message.component}"
                )
        else:
            raise ValueError(f"Invalid message for SimpleChatbot component: {chat_message}")
    ...
    def _create_file_message(chat_message, filepath):
        ...
        return FileMessage(
            ...
            alt_text=SimpleChatbot._get_alt_text(chat_message),
        )
    ...
    def _postprocess_message_messages(
        ...
        if isinstance(message, dict):
            ...
            raise Error(
                f"Invalid message for SimpleChatbot component: {message}", visible=False
            )
    ...
    def postprocess(
        ...
        """
        Parameters:
            value: If type is `tuples`, expects a `list[list[str | None | tuple]]`, i.e. a list of lists. The inner list should have 2 elements: the user message and the response message. The individual messages can be (1) strings in valid Markdown, (2) tuples if sending files: (a filepath or URL to a file, [optional string alt text]) -- if the file is image/video/audio, it is displayed in the SimpleChatbot, or (3) None, in which case the message is not displayed. If type is 'messages', passes the value as a list of dictionaries with 'role' and 'content' keys. The `content` key's value supports everything the `tuples` format supports.
        Returns:
